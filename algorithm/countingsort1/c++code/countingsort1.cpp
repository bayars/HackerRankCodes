/*
Hacker rank link: https://www.hackerrank.com/challenges/countingsort1/problem
...
It's doesn't work now but works only first and second test. Hacker rank say congrulations what i made. it's don't work for big decimal number give segmentaion fault error. Vector size 10000 should be require too much memory.
...
vector<int> countingSort(vector<int> arr) {
    map<int,int> number_occurence;

    for(int i : arr) number_occurence[i]++;

    auto it = number_occurence.end();
    it--;

    arr.resize(it->first + 1);

    for(int i = 0; i < arr.size(); i++)
        arr[i] = number_occurence[i];
