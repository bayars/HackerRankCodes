...
[Problem Link](https://www.hackerrank.com/challenges/ruby-enumerable-any-all-none-find/problem?h_r=next-challenge&h_v=zen)
...
def func_any(hash)
    ...
    hash.any? {| t , p |  t.is_a?(Integer)}
end

def func_all(hash)
    # Check and return true if all the values within the hash are Integers and are < 10
    # If not all values satisfy this, return false.
    hash.all? {| t , p | p.is_a?(Integer) &&  p < 10 }
end
...
def func_find(hash)
    ...
    hash.find do |k, v| 
        ...
        (k.is_a?(String) && v.is_a?(String) && v[0] == 'a')
    end
end
