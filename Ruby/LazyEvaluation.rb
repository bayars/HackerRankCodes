...
Problem link = https://www.hackerrank.com/challenges/ruby-lazy/problem

Lazy evaluation is an evaluation strategy that delays the assessment of an expression until its value is needed.

Ruby

introduced a lazy enumeration feature. Lazy evaluation increases performance by avoiding needless calculations, and it has the ability to create potentially infinite data structures.
...
You are not given how big

is.

Output Format

Print an array of the first
...
p Prime.each.lazy.select { |x| x == x.to_s.reverse.to_i }.first(t)
